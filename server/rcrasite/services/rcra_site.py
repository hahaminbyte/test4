...
# Fixed EPA IDs for local demo manifests (no EPA account required).
LOCAL_DEMO_HANDLERS = (
    {
        "epa_id": "MOCKTRANS001",
...
@transaction.atomic
def ensure_local_demo_handlers() -> list[RcraSite]:
    """Create local mock transporter/TSDF handlers if missing (for offline demo use)."""
    created: list[RcraSite] = []
    for spec in LOCAL_DEMO_HANDLERS:
        if RcraSite.objects.filter(epa_id=spec["epa_id"]).exists():
            continue
        address = Address.objects.create(
            street_number="200",
            address1="Mock Handler Blvd",
            city="Arlington",
            state="VA",
            country="US",
            zip="22202",
        ...
        phone = RcraPhone.objects.create(number="703-555-0199")
        ...
        created.append(site)
        logger.info("Created local demo handler %s", site.epa_id)
    return created
