...
the [compose.yaml](https://github.com/USEPA/haztrak/blob/main/compose.yaml)
file._
...
- With `npm start`, an empty database is migrated and seeded automatically.
  Seed accounts use password `password1` (`testuser1`, `admin`, `orgadmin`).
- You can also register a new account at `/register`.

Fixtures include sample sites:
...
Haztrak's [compose.yaml](/compose.yaml) file will load fixtures,
...
Haztrak can run without Docker as long as you have the required dependencies
installed locally:

- PostgreSQL (e.g. on `localhost:5432`)
- Redis (on `localhost:6379`)
- Python (3.12+) and Node

1. (Optional for the frontend) Copy env for Vite: `cp configs/.env.local client/.env`
2. Export env vars for the backend in your shell:
   `set -a; source configs/.env.local; set +a`
3. Prepare the database and apply migrations: `./runhaz.sh -d`
4. Load dev fixtures: `./runhaz.sh -l`
5. Start the Django server: `uv run python server/manage.py runserver`
6. In separate terminals (repeat step 2), start background tasks:
   - Celery worker: `uv run python server/manage.py celery_worker`
   - Celery beat: `uv run python server/manage.py celery_beat`
