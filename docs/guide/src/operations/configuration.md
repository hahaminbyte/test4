...
Haztrak also comes with Dockerfiles and a Docker Compose file,
including [compose.yaml](/compose.yaml). Environment variables can
be passed
to a
using [docker compose --env-file flag](https://docs.docker.com/compose/environment-variables/#using-the---env-file--option)
like so.

```shell
docker compose -f compose.yaml --env-file configs/.env.dev up --build
```
