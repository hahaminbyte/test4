...
import { Button, Col, Container, Form, ListGroup, Row, Stack, Alert } from 'react-bootstrap';
...
import { addTask, updateTask, useAppDispatch, useGetCurrentUserQuery, useSignEManifestMutation } from '~/store';
...
export function QuickerSignForm({ mtn, mtnHandler, handleClose, siteType }: QuickerSignProps) {
  ...
  const { register, handleSubmit, setValue } = useForm<QuickerSignature>({
    defaultValues: {
      printedSignatureName: defaultName,
  ...
  useEffect(() => {
    if (defaultName) {
      setValue('printedSignatureName', defaultName);
    }
    ...
    <>
      {ApiError ? (
        <Alert variant="danger" className="mb-3">
          Signature request failed. Check RCRAInfo credentials and try again.
        </Alert>
      ) : null}
