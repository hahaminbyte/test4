...
const renderLegend = (props: any): ReactElement => {
  const { payload, handleMouseEnter, handleMouseLeave, handleClick, chartData = [] } = props;

  return (
    <div className="tw-relative tw-flex tw-flex-wrap tw-justify-center">
      {payload.map((entry: any, index: number) => {
        const dataEntry = chartData.find((d: Entry) => d.name === entry.value);
        ...
        return (
          <button
            ...
            onClick={dataEntry ? handleClick(dataEntry) : undefined}
...
export function ManifestStatusPieChart({ data = [] }: { data?: Entry[] }) {
  const [activeIndex, setActiveIndex] = useState(-1);
  const [animationIsActive, setAnimationIsActive] = useState(true);
  const navigate = useNavigate();

  if (!data.length) {
    return <p className="text-muted text-center mb-0 py-5">No status data yet.</p>;
  }
