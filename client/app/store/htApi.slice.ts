...
export const haztrakApi = createApi({
  ...
  endpoints: (build) => ({
    ...
    createOrg: build.mutation<Organization, { name: string }>({
      query: (data) => ({
        url: 'orgs/create',
        method: 'POST',
        data,
      }),
      invalidatesTags: ['org', 'profile'],
    }),
    createSampleSite: build.mutation<HaztrakSite, { orgSlug: string }>({
      query: ({ orgSlug }) => ({
        url: `orgs/${orgSlug}/sites/sample`,
        method: 'POST',
      }),
      invalidatesTags: ['site', 'profile', 'org', 'manifest'],
    }),
    // eslint-disable-next-line @typescript-eslint/no-invalid-void-type
    ...
    getDashboardStats: build.query<
      {
        byStatus: { name: string; value: number; searchParam: string }[];
        byMonth: { date: string; hazardous: number; nonHazardous: number }[];
        generatorStatus: { day: number; haz: number }[];
