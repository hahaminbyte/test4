...
export function Profile(): ReactElement {
  ...
  const { data: rcrainfoProfile } = useGetRcrainfoProfileQuery(user?.username ?? '', {
    skip: !user?.username,
  });
        ...
        <Row className="tw-flex tw-justify-center">
          <Col xs={12} md={10} lg={8}>
            <HtCard title="RCRAInfo Profile">
              <HtCard.Body>
                {rcrainfoProfile ? (
                  <RcraProfile profile={rcrainfoProfile} />
                ) : (
                  <p className="text-muted mb-0">Loading RCRAInfo profile…</p>
                )}
              </HtCard.Body>
            </HtCard>
          </Col>
